#!/usr/bin/env nu
use ~/.dotfiles/xpkg/utils/is-compatible.nu
use ~/.dotfiles/xpkg/utils/is-optional.nu
use ~/.dotfiles/xpkg/utils/yesno.nu
use ~/.dotfiles/xpkg/utils/escape-filename.nu
export def main [--allow-upgrade = false] {
    for manifest_file in (glob ~/.dotfiles/xpkg/packages/**/*.manifest.toml$ | where { |x| ($x | path type) == file }) {
