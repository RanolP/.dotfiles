    ...
    if not allow_prose:
        ...
        if prose:
            ...
            decide("deny", (
                ...
                "WHY: a reviewer reads a body to find where to look, not to read "
                "a narrative. Chaining clauses inside one item hides which single "
                "fact the item is making, and forces the reader to parse the "
                "sentence before they can pull the cause and the action out of it. "
                "One predicate per unit and an arrow chain put the cause, the "
                "effect and the action in fixed positions, so the item is read at "
                "a glance; a parenthesis carries what is true but not load-bearing.\n"
                "HOW: split the clauses into 원인 -> 결과 -> 조치, drop every "
                "conjugated connective, and leave one predicate standing per "
                "segment. Swapping 한다 for 함 changes the ending, not the "
                "structure, so it does not clear this.\n"
        ...
        if flat:
            ...
            decide("deny", (
                ...
                "WHY: N siblings at one level assert N independent facts. They are "
                "not independent -- one of them caused the rest -- so a flat list "
                "throws away the reasoning the author already did and hands the "
                "reviewer the job of rebuilding it. Nesting states that tree "
                "outright: the reviewer reads one root cause, takes the children as "
                "its consequences, and can accept or skip a whole subtree at once. "
                "Ordering by commit instead of by cause encodes the order the work "
                "happened in, which is the one ordering the reviewer does not need.\n"
                "HOW: name the root cause and put it alone at the top level, then "
                "indent every item that exists only because of it. 대국적으로 -- "
                "the 개요 compresses the same chain to one or two lines.\n"
                "  - 테스트 러너 없음 -> vitest 구성\n"
