...
1. Run `slopless --help` before the first Slopless run in the session.
...
slopless "docs/**/*.md" > ".slopless/findings/$(date +%Y-%m-%d-%H%M%S)--docs-review.json"
```

```bash
mkdir -p .slopless/findings
slopless draft.md > ".slopless/findings/$(date +%Y-%m-%d-%H%M%S)--draft.json"
```

```bash
mkdir -p .slopless/findings
slopless --stdin --stdin-filename draft.md > ".slopless/findings/$(date +%Y-%m-%d-%H%M%S)--stdin-draft.json"
```
