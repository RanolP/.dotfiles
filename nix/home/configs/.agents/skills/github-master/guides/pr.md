...
**One predicate per unit (개조식).** Chaining clauses inside one item hides which single fact the item is making, and makes the reader parse a sentence before they can pull the cause and the action out of it. An arrow chain (`->`) of noun phrases puts 원인, 결과 and 조치 in fixed positions instead, so each is found by position rather than by reading; a parenthesis carries what is true but not load-bearing. This is why swapping 한다→함 clears nothing (없어 … 동작하지 않던 … is still one sentence with the endings filed off) — what has to go is the chaining, not the ending.
