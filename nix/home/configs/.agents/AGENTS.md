...
- DO (scan the draft before sending): the caps above are a target, and a target alone gets missed, so run this literal search over the drafted reply and fix every hit -- `should`/`would`/`may`/`might`/`could` become `can`, `will`, `must`, or a plain statement of the fact; `'ll`/`'re`/`n't`/`it's` expand to full words; `has been`/`have been` become simple past or simple present; `, making`/`, allowing`/`, enabling`/`, ensuring` become a new sentence with a real subject; `;` becomes two sentences; `e.g.`/`i.e.`/`etc.` become "for example", "that is", or the named items; every `if` and `when` moves to the front of its own sentence, followed by a comma; then count the words in the three longest sentences and split each one over its cap
